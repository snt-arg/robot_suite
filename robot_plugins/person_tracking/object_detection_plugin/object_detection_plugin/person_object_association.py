...
class PersonObjectAssociation(PluginNode):
    ...
    def _init_publishers(self) -> None:
        ...
        self.publisher_tracking_info = self.create_publisher(
            String, self.tracking_info_topic, 5
        )

    def _init_subscriptions(self) -> None:
        """Method to initialize subscriptions"""
        self.sub_boxes = self.create_subscription(
            AllBoundingBoxes, self.bounding_boxes_topic, self.listener_callback, 5
        )
    ...
    def listener_callback(self, boxes_msg) -> None:
        ...
        # Logs
        self.get_logger().debug(f"Bounding boxes received N°{self.counter} received.")
        self.counter += 1
    ...
    def map_person_to_objects(self) -> None:
        ...
        if self.boxes is not None:
            ...
            possessor_to_objects_dict = assign_objects_to_persons(
                tmp_list_person, tmp_list_object, self.overlapping_method
            )

            # print(f"\n## Possessors : {possessor_to_objects_dict}\n") #<--- to debug

            # JSON string message to send to LLM command interpreter
            self.json_string_msg.data = construct_JSON_string(
                tmp_list_person, possessor_to_objects_dict
            )

            self.get_logger().debug(str(self.json_string_msg.data))

        else:
            self.get_logger().info(
                f"*** Didn't receive boxes to produce the mapping of person to objects ***\n"
            )
...
def main(args=None):
    ...
    # Node instantiation
    associator = PersonObjectAssociation("associator_node")
