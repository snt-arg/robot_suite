...
export function ChatArea({ msgList }) {
    ...
    const chatBoxes = msgList.map((element) => {
        return <li key={element.id} className={element.msgType === "sent" ? "msg-sent" : "msg-received"}>{element.text}</li>;
    });
...
export function useChatHistory(initMsgList) {
    ...
    const updateSentMsg = (msgSent) => {
        setMsgList(s => [...s, { id: idCounter, msgType: "sent", text: msgSent }]);
        setIdCounter(s => s + 1);

    }

    const updateReceivedMsg = (msgReceived) => {
        setMsgList(s => [...s, { id: idCounter, msgType: "received", text: msgReceived }]);
        setIdCounter(s => s + 1);
    }
