...
With the growing development of domestic robots and the over increasing use of robots in daily life, the need for more intuitive and natural Human-Robot Interaction (HRI) also increases. It is crucial to guarantee simple, casual human-robot interactions to ensure that robots are not only useful to advanced and technical people, but also to normal people.

In an effort to provide an intuitive interface for HRI, we leveraged the reasoning capabilities of Large Language Models (LLMs) via [ROSA](https://github.com/nasa-jpl/rosa) to develop a robot agent, acting as a bridge between humans and robots. Our robot agent lets users interact with robots via **text** and **voice** -based natural language commands. For instance, the user might say _move forward for 1 second_, and the robot agent, coupled with our underlying behaviour tree architecture will instruct the robot to do so, if possible.
