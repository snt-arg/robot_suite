...
class VoiceInOut(Node):
    ...
    def __init__(self):
        ...
        for i in range(self.pya.get_device_count()):
            ...
            if re.match(re.compile(r"^DJI\sMIC\sMINI", re.I), audio_source_name):
                self.get_logger().info(
                    f"\033[95m Detected a Dji mic mini, we will use it.\033[00m"
                )
                mic_index = i
                mic_sample_rate = int(
                    self.pya.get_device_info_by_index(i)["defaultSampleRate"]
                )
        ...
        self.stt_mic = sr.Microphone(
            sample_rate=mic_sample_rate, device_index=mic_index
        )
